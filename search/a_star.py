...
def astar_search(
    ...
    while open:
        ...
        if pop_g == state_cost[pop_state]:
            ...
            expansions += 1
            logging.info("Updated expansions counter %d" % (expansions))
        ...
        for op, neighbor_state in task.get_successor_states(pop_state):
            neighbor_node = searchspace.make_child_node(pop_node, op, neighbor_state)
            h = heuristic(neighbor_node)
            if h == float("inf"):
                continue
            lowest_known_cost = state_cost.get(neighbor_state, float("inf"))
            if neighbor_node.g < lowest_known_cost:
                ...
                node_tiebreaker += 1
                heapq.heappush(open, make_open_entry(neighbor_node, h, node_tiebreaker))
                state_cost[neighbor_state] = neighbor_node.g
